...
/** Shared write-ahead delivery for the two native choice activities. File format remains version 1.
 * Worker-thread confined; activity content/generation stays outside this persistence boundary. */
open class DurableSessionHost(private val journal: ProgressStorage, private val progress: ProgressRepository,
    ...
    private fun start(id: SessionId, round: RoundLength, seed: Long, language: ContentLanguage): List<SessionEffect> {
        val generated = generate(id, round, seed) as? GenerationResult.Generated ?: throw IOException("Invalid content")
        val transition = SessionReducer.start(generated.plan, language, content)
    ...
    private fun decode(bytes: ByteArray): Pair<SessionState, List<ProgressEvent>> {
        ...
        return DataInputStream(ByteArrayInputStream(payload)).use { input ->
            ...
            require(events.all { it.origin.session == restored.state.plan.id && it.origin.activity == activity })
