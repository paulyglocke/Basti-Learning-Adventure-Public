...
class WilmaOrderHost(private val disk:ProgressStorage,private val progress:ProgressRepository) {
    var state:WilmaOrderState?=null; private set
    ...
    private fun start(id:SessionId,seed:Long,language:ContentLanguage):SessionEffect.Narrate {
        val n=WilmaOrder.start(id,seed,language);save(n,emptyList());state=n;pending=emptyList();failure=false
        return SessionEffect.Narrate(WilmaOrder.prompt(n),NarrationKind.INSTRUCTION)
    }
    fun dispatch(action:WilmaOrderAction):SessionEffect.Narrate? {
        val before=state ?: return null
        if(failure || hasPending) return null
        val next=WilmaOrder.reduce(before,action)
    ...
    private fun save(s:WilmaOrderState,events:List<ProgressEvent>) {
    ...
    companion object {
        internal fun encode(s:WilmaOrderState,events:List<ProgressEvent>):ByteArray {
            val buffer=ByteArrayOutputStream()
            DataOutputStream(buffer).use {o ->
                o.writeInt(1);o.writeInt(WilmaContent.REVISION);o.writeInt(WilmaContent.repository.version.schema);o.writeInt(WilmaContent.repository.version.revision)
        ...
        internal fun decode(bytes:ByteArray):Pair<WilmaOrderState,List<ProgressEvent>> {
            ...
            return DataInputStream(ByteArrayInputStream(payload)).use {i ->
                require(i.readInt()==1 && i.readInt()==WilmaContent.REVISION && i.readInt()==WilmaContent.repository.version.schema && i.readInt()==WilmaContent.repository.version.revision)
                val id=SessionId(i.readUTF());val language=ContentLanguage.valueOf(i.readUTF());val acknowledged=i.bool()
                val choices=List(7){ContentId(i.readUTF())}
                val steps=List(7){TaskProgress(AnswerState.valueOf(i.readUTF()),i.readInt(),i.readInt(),SupportUse(i.readInt(),i.bool(),i.bool()),i.readUTF().takeIf {it.isNotEmpty()}?.let(::ContentId))}
                val s=WilmaOrderState(id,language,choices,steps,acknowledged)
                ...
                events.forEach { e -> when(e) {
                    is AttemptEvent -> {
                        ...
                        require(index in 0..6 && (index==s.index || index==s.index-1) && s.steps[index].answer!=AnswerState.UNANSWERED)
                        require(e==WilmaOrder.attempt(s,index))
                    }
                    is CompletionEvent -> require(s.completed && !s.acknowledged && e==WilmaOrder.completion(s))
                } }
