...
class VocabularyHostTest(private val phase: VocabularyPhase) {
    companion object {
        @JvmStatic @org.junit.runners.Parameterized.Parameters(name="{0}")
        fun phases() = listOf(arrayOf(VocabularyPhase.FIND), arrayOf(VocabularyPhase.NAME))
    }
    ...
    private fun open(storage: ProgressStorage, round: RoundLength = RoundLength.FIVE) = VocabularyContent.host(phase,storage,repository).also {
    ...
    @Test fun unansweredRestoreIsSilentAndDoesNotRegenerate() {
        val disk=storage();val host=open(disk)
        val other=VocabularyContent.host(phase,disk,repository)
    ...
    @Test fun realProgressRepositoryAndJournalSurviveRecreationTogether() {
        ...
        fun host()=VocabularyContent.host(phase,journal,ProgressFixtures.repository(progressDirectory)).also {
        ...
        assertTrue(records.all {it.origin.activity==VocabularyContent.activity(phase)})
        assertTrue(records.all {it.evidence.skill==h.state!!.task.question.skill && it.evidence.context==h.state!!.task.question.context})
        assertEquals(listOf(AttemptOutcome.INCORRECT,AttemptOutcome.CORRECT),records.map {it.outcome})
    }
