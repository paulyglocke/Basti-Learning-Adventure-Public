...
class SeasonsHostTest {
    ...
    private fun open(storage: ProgressStorage, round: RoundLength = RoundLength.FIVE) = SeasonsContent.host(storage,repository).also {
    ...
    @Test fun unansweredRestoreIsSilentAndDoesNotRegenerate() {
        val disk=storage();val host=open(disk)
        val other=SeasonsContent.host(disk,repository)
    ...
    @Test fun realProgressRepositoryAndJournalSurviveRecreationTogether() {
        ...
        fun host()=SeasonsContent.host(journal,ProgressFixtures.repository(progressDirectory)).also {
