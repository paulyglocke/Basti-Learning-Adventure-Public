...
- Integrate native session restoration/lifecycle with the first migrated screen and build shared settings/completion UI components. Native progress storage/deduplication is implemented separately; live delivery integration, reward deduplication and physical recovery acceptance remain requirements; the framework does not complete those parts of this P1 item.
...
- Integrate native progress delivery/checkpoint retry with the first migrated host; retain undelivered effects, handle capacity/schema failures and validate Android process/filesystem behavior. Dashboard, rewards and broader retention/migration policy remain separate work.
