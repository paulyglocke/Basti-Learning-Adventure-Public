...
- First text-answer choice slice is implemented in Prepositions and Seasons recognition/next/before: NativeTextChoice shares neutral geometry, wrapping and Material interaction semantics; callbacks/locking and separate Listen remain caller-owned. Physically review the neutral choice/forward-action distinction and large-text scrolling. Vocabulary's text-only What Is It? / NAME choices now also use NativeTextChoice with the existing 72dp minimum; FIND glyph choices and separate Listen stay custom. Image-choice contracts, headers, progress markers and other visual-system work remain unfinished. Do not genericize Wilma colour cues or invent correctness states.
- Before adding another native activity, audit native Android TTS ownership. Current live native ViewModels each own an `AndroidSystemSpeechEngine` while the legacy shell also owns TTS; do not add another eager engine blindly. Prefer the smallest tested ownership/lazy-init change that preserves the existing `AudioController` owner/session cancellation contract. This is infrastructure cleanup, not a new audio engine or policy rewrite.
